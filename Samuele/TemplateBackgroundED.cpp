...
double GetAngReso(TF1*AngReso, double Eele){
  ...
  if(Eele<290){ //215 NID   290 ED
    return AngReso->Eval(Eele);
  } else {
    return 5; //3 degrees NID     5 ED
  }
...
int main(int argc, char** argv){
  ...
  TFile* fBkg= TFile::Open("/nfs/cygno2/users/torellis/CYGNO_30_Sim/build/outfiles_V4/NormalizedHisto_UPKapton.root");
  THStack* stack;
  ...
  TFile* fOutTemplate = new TFile(Form("Out_V9_UltrapureKapton/OutTemplate_Bkg_%s.root",argv[1]),"recreate");
  fOutTemplate->cd();
  ...
  TFile* fOutTemplate_test = new TFile(Form("Out_V9_UltrapureKapton/OutTemplate_Bkg_%s_test.root",argv[1]),"recreate");
  fOutTemplate_test->cd();
  HistoERecoTot->Write();
  HistoCosThetaTot->Write();
