...
void GenSignal(){
  ...
  TH1D* HistoCosThetaTot = new TH1D("HistoCosThetaTot","HistoCosThetaTot",100,-1,1);
  ...
  */
  ...
  for(int i=0;i<NTries;i++){
    ...
    if(Eele>10){
      ...
      Theta = TMath::ACos(cosTheta)/TMath::Pi()*180* (arc4random() % 2 ? 1 : -1)+90;
  ...
  TFile* fOutSignal = new TFile("OutSignal.root","recreate");
  fOutSignal->cd();
  ...
  TFile* fOutSignal_test = new TFile("OutSignal_test.root","recreate");
  fOutSignal_test->cd();
  HistoERecoTot->Write();
