...
Double_t SmearTheta(Double_t Theta, TRandom* r){

  Double_t p0 = 3.8504870;
  Double_t p1 = 516.10439;

  TF1* AngReso = new TF1("AngReso","[0]+[1]/x");
  AngReso->SetParameter(0,p0);
  AngReso->SetParameter(1,p1);

  Double_t ThetaSmeared = r->Gaus(Theta,AngReso->Eval(Theta));
...
int main(int argc, char** argv){
  ...
  TF1* AngReso = new TF1("AngReso","[0]+[1]/x");
  AngReso->SetParameter(0,3.8504870);
  AngReso->SetParameter(1,516.10439);
  ...
  for(int i=0;i<BinHistoEReso.size()-2;i++){
    ...
    NbinsX=(int)( 180/(2*AngReso->Eval(ECenter)) );
    if(NbinsX<3) NbinsX=3;
    AngHistogram[i] = new TH2D(Form("Histo%.0f",ECenter),Form("Histo%.0f",ECenter),NbinsX,0,180,(int)( 360/(2*AngReso->Eval(ECenter)) ),-180,180);
    AngHistogram[i]->GetXaxis()->SetTitle("Theta [deg]");
    AngHistogram[i]->GetYaxis()->SetTitle("Phi [deg]");

    AngHistogram_test[i] = new TH2D(Form("Histo%.0f_test",ECenter),Form("Histo%.0f_test",ECenter),60,0,180,60,-180,180);
  ...
  for(int i=0;i<BinHistoEReso.size()-2;i++){
    ...
    ECenter = (BinHistoEReso[i]+BinHistoEReso[i+1])/2;
    NbinsX=(int)( 360/(2*AngReso->Eval(ECenter)) );
    CosBin.clear();
    ...
    AngHistogramCos[i]= new TH2D(Form("HistoCos%.0f",ECenter),Form("HistoCos%.0f",ECenter),CosBin.size()-1,&CosBin[0],(int)( 360/(2*AngReso->Eval(ECenter)) ),-180,180);
    AngHistogramCos[i]->GetXaxis()->SetTitle("Theta [deg]");
    AngHistogramCos[i]->GetYaxis()->SetTitle("Phi [deg]");

    AngHistogramCos_test[i]= new TH2D(Form("HistoCos%.0f_test",ECenter),Form("HistoCos%.0f_test",ECenter),60,-1,1,60,-180,180);
    AngHistogramCos_test[i]->GetXaxis()->SetTitle("Theta [deg]");
  ...
  */
  ...
  TFile* fOutTemplate = new TFile(Form("OutHighStat/OutTemplate%s.root",argv[1]),"recreate");
  fOutTemplate->cd();
  ...
  TFile* fOutTemplate_test = new TFile(Form("OutHighStat/OutTemplate_test%s.root",argv[1]),"recreate");
